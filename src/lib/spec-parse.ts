...
export function parseSpec(source: string): ParseResult {
  ...
  while (i < bodyLines.length) {
    ...
    // statement body: from after the status line until the next heading
    const bodyStart = statusMatch ? j + 1 : i + 1;
    let k = bodyStart;
    ...
    assertions.push({ humanId, title, status, statement, order: order++ });
    i = k;
  }
